import express, { Router, Request, Response } from "express";
import bodyParser from "body-parser";

import { Car, cars as cars_list } from "./cars";
import { filter } from "bluebird";

(async () => {
  let cars: Car[] = cars_list;

  //Create an express applicaiton
  const app = express();
  //default port to listen
  const port = 8082;

  //use middleware so post bodies
  //are accessable as req.body.{{variable}}
  app.use(bodyParser.json());

  // Root URI call
  app.get("/", (req: Request, res: Response) => {
    res.status(200).send("Welcome to the Cloud!");
  });

  // Get a greeting to a specific person
  // to demonstrate routing parameters
  // > try it {{host}}/persons/:the_name
  app.get("/persons/:name", (req: Request, res: Response) => {
    let { name } = req.params;

    if (!name) {
      return res.status(400).send(`name is required`);
    }

    return res.status(200).send(`Welcome to the Cloud, ${name}!`);
  });

  // Get a greeting to a specific person to demonstrate req.query
  // > try it {{host}}/persons?name=the_name
  app.get("/persons/", (req: Request, res: Response) => {
    let { name } = req.query;

    if (!name) {
      return res.status(400).send(`name is required`);
    }

    return res.status(200).send(`Welcome to the Cloud, ${name}!`);
  });

  // Post a greeting to a specific person
  // to demonstrate req.body
  // > try it by posting {"name": "the_name" } as
  // an application/json body to {{host}}/persons
  app.post("/persons", async (req: Request, res: Response) => {
    const { name } = req.body;

    if (!name) {
      return res.status(400).send(`name is required`);
    }

    return res.status(200).send(`Welcome to the Cloud, ${name}!`);
  });

  // @TODO Add an endpoint to GET a list of cars
  // it should be filterable by make with a query paramater
  app.get("/cars/", async (req: Request, res: Response) => {
    const { make } = req.query;
    if (make) {
      let filteredCars = cars.filter(function (item) {
        return item.make == make;
      }, cars);
      return res.status(200).send(filteredCars);
    }
    return res.status(200).send(cars);
  });

  // @TODO Add an endpoint to get a specific car
  // it should require id
  // it should fail gracefully if no matching car is found
  app.get("/cars/:id", async (req: Request, res: Response) => {
    const { id } = req.params;

    const filteredCars = cars.filter((item) => item.id == id);

    if (filteredCars.length > 0) {
      return res.status(200).send(filteredCars);
    }
    return res.status(404).send(`car not found`);
  });

  app.get("/cars", async (req: Request, res: Response) => {
    const { id } = req.query;
    const filteredCars = cars.filter((car) => car.id === id);
    if (filteredCars.length > 0) {
      return res.status(200).send(filteredCars);
    }
    return res.status(404).send(`car not found`);
  });

  // simple function to check if a car id is valid
  function validateCarAndReturn(id: any, res: Response) {
    if (isNaN(id)) {
      return res.status(400).send(`id invalid`);
    }
    let carId: number = parseInt(<string>id);
    if (carId < 1) {
      return res.status(400).send(`id invalid`);
    }
    if (carId >= cars.length || typeof cars[carId] == "undefined") {
      return res.status(404).send(`id does not exist`);
    }
    return res.status(201).send(cars[carId]);
  }

  /// @TODO Add an endpoint to post a new car to our list
  // it should require id, type, model, and cost

  app.post("/cars", async (req: Request, res: Response) => {
    const { id, type, model, make, cost } = req.body;
    if (!id) {
      return res.status(400).send(`id required`);
    }
    if (!make) {
      return res.status(400).send(`make required`);
    }
    if (!type) {
      return res.status(400).send(`type required`);
    }
    if (!model) {
      return res.status(400).send(`model required`);
    }
    if (!cost) {
      return res.status(400).send(`cost required`);
    }
    if (isNaN(id) || id <= cars.length || typeof cars[id] != "undefined") {
      return res.status(400).send(`id already exists`);
    }
    let newCar: Car = {
      make: make,
      type: type,
      model: model,
      cost: cost,
      id: id,
    };
    cars.push(newCar);
    return res.status(200).send(newCar);
  });

  // Start the Server
  app.listen(port, () => {
    console.log(`server running http://localhost:${port}`);
    console.log(`press CTRL+C to stop server`);
  });
})();
